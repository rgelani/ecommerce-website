...
class HsnCodeController extends Controller
{
    ...
    {
        $hsncode = HsnCode::with("childSubcategory")->get();
        // $hsncode = HsnCode::get();
        if ($request->ajax()) {
            return DataTables::of($hsncode)
                ->addIndexColumn()
                ->addColumn('childSubcategory', function ($row) {
                    $etc = json_decode($row->childSubcategory);
                    if (!isset($etc->name)) {
                        return "Not available";
                    }
                    return "$etc->name";
                })
                ->rawColumns(['childSubcategory'])
                ->addColumn('action', function ($row) {
                    $edit = route('admin.hsncode.edit', ['id' => $row->id]);
                    $delete = route('admin.hsncode.delete', ['id' => $row->id]);
                    return "
                    <a href='{$edit}' class='mr-md-1'>
        ...
        $chilSubCategory = ChildSubCategory::get();
        return view('admin.hsncode.hsn', compact('hsncode', 'chilSubCategory'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            // $code_array = explode(",",$request->code);                    dd($sub_category->id);

            // create hsn code 
            foreach ($request->hsn_code as $key => $val) {
                HsnCode::create([
            ...
            \Log::info('Error on HsnCodeController storeHSN function : Message : ' . $e->getMessage() . ' | Line : ' . $e->getLine() . ' | File : ' . $e->getFile());
        }
    ...
    public function show($id)
    {
        try {
            ...
            $chilSubCategory = ChildSubCategory::get();
            return view('admin.hsncode.edithsn', compact('hsncode', 'chilSubCategory'));
        } catch (\Exception $e) {
            \Log::error('Error on HsnCodeController show function : Message : ' . $e->getMessage() . ' | Line : ' . $e->getLine() . ' | File : ' . $e->getFile());
        }
    }
    ...
    public function update($id, Request $request)
    {
        ...
        try {
            HsnCode::where("id", $id)->update([
                'child_sub_category_id' => $request->child_sub_category_id,
                "code" => $request->hsn_code
            ]);
        } catch (\Exception $e) {
            \Log::info('Error on HsnCodeController updateHSN function : Message : ' . $e->getMessage() . ' | Line : ' . $e->getLine() . ' | File : ' . $e->getFile());
        }
        return redirect('admin/hsncode');
    }
    ...
    {
        try {
            $response = HsnCode::where("id", $id);
            $response->delete();
        } catch (\Exception $e) {
            \Log::info('Error on HsnCodeController deleteHSN function : Message : ' . $e->getMessage() . ' | Line : ' . $e->getLine() . ' | File : ' . $e->getFile());
        }
        return redirect('admin/hsncode');
    }
}
