...
use App\Exports\BulkProductSheet;
use Response;
...
class CategoryController extends Controller
{
        ...
        if ($request->ajax()) {
            return DataTables::of($categories)
                ...
                ->addColumn('action', function ($row) {
                    $edit = route('admin.category.edit', ['id' => $row->id]);
                    $delete = route('admin.category.delete', ['id' => $row->id]);
    ...
    public function store(Request $request) {
        try {
            ...
            $category = Category::create(['name' => $request->category_name, 'image' => $fileName]);
        } catch(\Exception $e) {
            \Log::error('Error on categoryController categoryMainCreate function : Message : '.$e->getMessage().' | Line : '.$e->getLine().' | File : '.$e->getFile());
        }
    ...
    public function edit($id) {
        try {
            ...
            \Log::error('Error on categoryController edit function : Message : '.$e->getMessage().' | Line : '.$e->getLine().' | File : '.$e->getFile());
        }
        return redirect('admin/category/list');
    }
    ...
    public function delete($id) {
        try {
            ...
            \Log::error('Error on categoryController delete function : Message : '.$e->getMessage().' | Line : '.$e->getLine().' | File : '.$e->getFile());
        }
        return redirect('admin/category/list');
    }
