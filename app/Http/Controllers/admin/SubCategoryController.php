...
class SubCategoryController extends Controller
{
    public function index(Request $request) {
        ...
        return view('admin.subcategory.list', compact('categories'));
    }
        ...
        if ($request->ajax()) {
            return DataTables::of($categories)
                ...
                ->addColumn('action', function ($row) {
                    $edit = route('admin.subcategory.edit', ['id' => $row->id]);
                    $delete = route('admin.subcategory.delete', ['id' => $row->id]);
    ...
    public function store(Request $request) {
        try {
            if(!empty($request->category_id) && !empty($request->category_name)) {
                SubCategory::create(['name' => $request->category_name, 'category_id' => $request->category_id]);
            }
        } catch(\Exception $e) {
            dd($e);
            \Log::error('Error on subcategoryController categoryMainCreate function : Message : '.$e->getMessage().' | Line : '.$e->getLine().' | File : '.$e->getFile());
        }
    ...
    public function edit($id) {
        try {
            ...
            \Log::error('Error on subcategoryController edit function : Message : '.$e->getMessage().' | Line : '.$e->getLine().' | File : '.$e->getFile());
        }
    ...
    public function delete($id) {
        try {
            ...
            \Log::error('Error on subcategoryController delete function : Message : '.$e->getMessage().' | Line : '.$e->getLine().' | File : '.$e->getFile());
        }
        return redirect('admin/subcategory/list');
    }
}
