...
class BannerImagesController extends Controller
{
    public function index(Request $request) {
        return view('admin.bannerimage.list');
    }
    ...
    public function list(Request $request) {
        ...
        if(isset($request->type)) {
            $images = BannerImage::where('type', $request->type)->get();
            $type = $request->type;
        } else {
        ...
        if ($request->ajax()) {
            return DataTables::of($images)
                ...
                ->addColumn('action', function ($row) {
                    $edit = route('admin.bannerImage.edit', ['id' => $row->id]);
                    $delete = route('admin.bannerImage.delete', ['id' => $row->id]);
                    return "<ul class='d-flex p-0 category-action'>
                    <li>category
                        <a href='{$edit}'>
        ...
        return view('admin.bannerImage.list', compact('categories', 'type'));
    }
    ...
    public function store(Request $request) {
        try {
            ...
            \Log::error('Error on BannerImagesController store function : Message : '.$e->getMessage().' | Line : '.$e->getLine().' | File : '.$e->getFile());
        }
        return redirect('admin/banner-image/list?type='.$request->type);
    }

    public function edit($id) {
        try {
            $categories = ChildSubCategory::get();
            $image = BannerImage::where('id', $id)->first();
            return view('admin.bannerImage.edit', compact('categories', 'image'));    
        } catch(\Exception $e) {
            \Log::error('Error on BannerImagesController edit function : Message : '.$e->getMessage().' | Line : '.$e->getLine().' | File : '.$e->getFile());
        }
        return redirect('admin/banner-image/list');
    }    
    ...
    public function update(Request $request, $id) {
        try {
            ...
            BannerImage::where('id', $id)->update($data);
        } catch(\Exception $e) {
            \Log::error('Error on BannerImagesController edit function : Message : '.$e->getMessage().' | Line : '.$e->getLine().' | File : '.$e->getFile());
        }
        return redirect('admin/banner-image/list?type='.$request->type);
    ...
    public function delete($id) {
        try {
            ...
            \Log::error('Error on BannerImagesController delete function : Message : '.$e->getMessage().' | Line : '.$e->getLine().' | File : '.$e->getFile());
        }
        return redirect('admin/banner-image/list');
    }
}
