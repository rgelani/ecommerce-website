...
<div class="search_and_table_wrap">
    ...
    <div class="d-flex justify-content-end py-4">
        <button class="common_btn px-3" data-bs-toggle="modal" id="accept_order">accept order</button>
    </div>
...
$('#accept_order').click(function (e) {
    e.preventDefault();
    var ids = [];
    $.each($("input[name='new_order_id']:checked"), function () {
                ids.push($(this).val());
            });
            $.ajax({
                type: "POST",
...
function unacceptedOrder(id) {
    $.ajax({
        type: "POST",
        url: '{{ route('seller.order.unaccepted') }}',
        data: {
            id: id,
            status: $(this).data('status'),
            _token: "{{ csrf_token() }}"
