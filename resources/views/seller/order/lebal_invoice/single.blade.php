    ...
    <div class="tax_invoice_main">
        <div class="w-100">
            ...
            <div class="logo_invoice pt-5">
                <img src="{{ public_path('seller-assets/image/logo.png')??'' }}" class="" alt="" width="80px"
            ...
            <div class="table_invoice">
                ...
                <div class="w-100b " style="float: left">
                    ...
                    <ul>
                        ...
                        <li>{{ @$lineItem->order->address->address3??'' }} </li>
                        <li>{{ @$lineItem->order->address->city_id??'' }} {{ @$lineItem->order->address->pin_code??'' }}
                        </li>
                        <li>{{ @$lineItem->order->address->state_id??'' }}</li>
                        <li>{{ @$lineItem->order->address->country_id??'' }} {{ @$lineItem->order->address->pin_code??''
                            }}
                        </li>
                        {{--
                        <li>Gujarat</li>
                ...
                <div class="w-100 " style="float: left;">
                    ...
                    <ul style=" text-align: right;" class="border_middle">
                        ...
                        <li>{{ @$lineItem->seller->pickup_address??'' }} </li>
                        <li>{{ @$lineItem->seller->city_id??'' }} {{ @$lineItem->seller->pin_code??'' }}</li>
                        <li>{{ @$lineItem->seller->state_id??'' }}</li>
                        <li>{{ @$lineItem->seller->country_id??'' }} {{ @$lineItem->seller->pin_code??'' }}</li>
                    </ul>
                </div>
                ...
                <table class="w-100 border-bottom-0 border_bottom_top_tr">
                    ...
                    <tr class="pb-4 mb-4">
                        ...
                        <td style="width: 200px; text-align: center;">
                            <p class="">{{ @$lineItem->title??'' }}</p>
                            <span>SKU : {{ @$lineItem->sku??'' }}</span>
                        </td>
                        <td class="">{{ @$lineItem->sku??'' }}</td>
                        <td class="">{{ @$lineItem->quantity??'' }}</td>
                        <td class="">Rs. {{ @$lineItem->MRP??'' }}</td>
                        <td class="">0.00</td>
                        <td class="">{{ @$lineItem->selling_price??'' }}</td>
            ...
            <p style="text-align: right">NET TOTAL (In Value) <span>RS.{{ @$lineItem->lineItem->selling_price
                    ??'' }}</span></li>
                <hr>
        </div>
